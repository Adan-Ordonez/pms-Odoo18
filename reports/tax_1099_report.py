    ...
    def _table_query(self):
        ...
        query = f"""
                    ...
                    inner join (
                        ...
                        where (((account_account.account_type = 'liability_credit_card' or account_account.account_type = 'asset_cash' or (account_account.name->>'en_US') ilike '%%outstanding%%' or
                          account_account.code_store::text LIKE '%%200404%%' or (account_account.account_type = 'liability_payable' and account_move_line.matching_number is not null))
                          and account_move_line.credit > 0) or (account_account.code_store::text LIKE '%%6.3%%' and account_move_line.debit > 0)) and account_move_line.parent_state = 'posted'
                         ) paml on aml.move_id = paml.paid_move_id

                    where res_partner.box_1099_id is not null
                    and account_account.account_type NOT IN ('liability_payable', 'asset_receivable')
                    and aml.debit > 0
                    and aml.parent_state = 'posted'
                    and aml.move_id IN (SELECT am.id FROM account_move am WHERE am.move_type IN ('in_invoice', 'in_refund'))
                    group by aml.id
