...
#    )
    ...
    related_bill_count = fields.Integer(string="Bill Count", readonly=True)
    total_purchases = fields.Float(string="Total Purchases", readonly=True, help="Sum of related bill amounts")
    total_profit = fields.Float(string="Total Profit", readonly=True, help="Invoice Amount - Total Purchases")
    profit_margin = fields.Float(string="Margin %", readonly=True, help="(Profit / Invoice Amount) * 100")
    ...
    def action_print_invoices(self):
        ...
        records = self.env['invoice.report'].browse(active_ids)
        invoice_ids = records.mapped('invoice_id').ids

        if not invoice_ids:
            from odoo.exceptions import UserError
            raise UserError("No related invoices found for the selected records.")

        report = self.env['ir.actions.report'].search([
            ('report_name', '=', 'pms.invoice_template_w_property')
        ], limit=1)

        if not report:
            from odoo.exceptions import UserError
            raise UserError("Invoice report not found.")

        return report.report_action(self.env['account.move'].browse(invoice_ids))
