...
class pms_inspections(models.Model):
    ...
    def create(self, vals_list):
        if 'address' in vals_list[0] and vals_list[0]['address']:
            property_id = vals_list[0]['address']
            property = self.env["pms.property"].browse(property_id)
            if property:
                _logger.info(property)
                _logger.info(property.utility_phase)
                vals_list[0]['construction_status'] = property.utility_phase

        record = super(pms_inspections, self).create(vals_list)
        return record
