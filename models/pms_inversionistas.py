    ...
    def create_entry(self):
        ...
        if not self.without_journal:
            if record_id.exit_strategy == 'liquidation':
                ...
                debit_capital = {
                    'account_id': self.env['account.account'].with_company(record_id.company_id.id).search([('code', '=', '2000404')], limit=1).id,
                ...
                debit_interest = {
                    'account_id': self.env['account.account'].with_company(record_id.company_id.id).search([('code', '=', '6000404')], limit=1).id,
    ...
    def create_entry(self):
        ...
        if not self.without_journal:
            journal_entry = self.env['account.move'].sudo().create({
                ...
                (0, 0, {
                    'account_id': self.env['account.account'].with_company(self.company.id).search([('code', '=', '2000404')], limit=1).id,
                    'debit': 0,
                    'credit': amount,
                    'partner_id': record_id.partner_id.id,
                    'analytic_distribution': {str(record_id.project.address.analytical_account.id): 100.0},
