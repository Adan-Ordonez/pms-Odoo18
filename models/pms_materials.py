...
class PMSMaterials(models.Model):
    ...
    created_to_waiting_cust = fields.Integer(aggregator='avg', default=False, string="Days from 'Creation Date' to 'Waiting Payment'", readonly=True, store=True, compute="calc_created_to_waiting_cust_days",
    ...
    wait_to_gave_pay = fields.Integer(aggregator='avg', default=False, string="Days from 'Payment Request' to 'Payment Date'", readonly=True, store=True, compute="calc_wait_to_gave_pay_days",
    ...
    inv_pay_to_ordered = fields.Integer(aggregator='avg', default=False, readonly=True, string="Days from 'Invoice Payment Date' to 'Ordered Date'", store=True, compute="calc_inv_pay_to_ordered_days",
    ...
    ordered_to_delivered = fields.Integer(aggregator='avg', default=False, readonly=True, string="Days from 'Ordered' to 'Delivered'", store=True, compute="delivered_to_ordered_calculator",
    ...
    # wait_to_inv_pay = fields.Integer(aggregator='avg', default=False, readonly=True, string="Days from 'Waiting Payment' to 'Invoice Payment Date'", store=True, compute="calc_wait_to_inv_pay_days",
    ...
    picking_id = fields.Many2one('stock.picking', string='Inventory Picking', readonly=True, copy=False, tracking=True)
    picking_count = fields.Integer(string='Picking Count', compute='_compute_picking_count')
    stock_move_ids = fields.One2many('stock.move', 'material_order_id', string='Stock Moves', readonly=True)
    inventory_deducted = fields.Boolean(string='Inventory Deducted', default=False, readonly=True, tracking=True)
    stock_reserved = fields.Boolean(string='Stock Reserved', default=False, readonly=True, tracking=True)
    stock_availability_status = fields.Selection([
        ('available', 'In Stock'),
        ('partial', 'Partially Available'),
        ('unavailable', 'Not Available'),
        ('unchecked', 'Not Checked'),
    ], string='Stock Availability', compute='_compute_stock_availability', store=True)
    
    def _find_storable_product(self, line):
        """Read-only lookup: find a matching storable product without creating one."""
        Product = self.env['product.product']
        if line.product:
            if line.product.type == 'product':
                return line.product
            storable = Product.search([('name', '=', line.product.name), ('type', '=', 'consu'), ('is_storable', '=', True)], limit=1)
            if storable:
                return storable
            return line.product
        if line.subproduct:
            storable = Product.search([('name', '=', line.subproduct.name), ('type', '=', 'consu'), ('is_storable', '=', True)], limit=1)
            if storable:
                return storable
        return False

    @api.depends('material_lines', 'material_lines.product', 'material_lines.subproduct',
                 'material_lines.quantity', 'stock_reserved')
    def _compute_stock_availability(self):
        for record in self:
            if not record.material_lines:
                record.stock_availability_status = 'unchecked'
                continue
            total_lines = 0
            available_lines = 0
            for line in record.material_lines:
                product = record._find_storable_product(line)
                if not product:
                    continue
                total_lines += 1
                if product.type != 'product':
                    continue
                qty_needed = line.quantity if line.quantity > 0 else 1
                if product.qty_available >= qty_needed:
                    available_lines += 1
            if total_lines == 0:
                record.stock_availability_status = 'unchecked'
            elif available_lines == total_lines:
                record.stock_availability_status = 'available'
            elif available_lines > 0:
                record.stock_availability_status = 'partial'
            else:
                record.stock_availability_status = 'unavailable'
    ...
    def action_check_stock_availability(self):
        ...
        if not self.material_lines:
            raise UserError(_("No material lines to check."))
        
        lines_info = []
        for line in self.material_lines:
            product = self._resolve_product_for_stock(line)
            if not product or product.type != 'product':
                continue
            qty_needed = line.quantity if line.quantity > 0 else 1
            qty_available = product.qty_available
            status = 'OK' if qty_available >= qty_needed else 'LOW'
            line_name = product.name
            lines_info.append(
                f"<li><b>{line_name}</b>: Need {qty_needed}, On Hand {qty_available} — "
                f"<span style='color: {'green' if status == 'OK' else 'red'}'>{status}</span></li>"
            )
        
        if lines_info:
            self.message_post(body=f"<b>Stock Availability Check:</b><ul>{''.join(lines_info)}</ul>")
        else:
            self.message_post(body="<b>Stock Availability Check:</b> No storable products found on material lines.")
        self._compute_stock_availability()

    def action_reserve_stock(self):
        self.ensure_one()
        if self.stock_reserved:
            raise UserError(_("Stock is already reserved for this order."))
        if self.order_status == 'delivered':
            raise UserError(_("Cannot reserve stock for a delivered order."))
        ...
        location_dest = self.env['stock.location'].search([('usage', '=', 'transit')], limit=1)
        ...
        picking = self.env['stock.picking'].sudo().create({
            ...
            'origin': f"{self.name or 'MO'} - Reservation",
        ...
        for line in self.material_lines:
            ...
            self.env['stock.move'].sudo().create({
                'name': f"{self.name or 'MO'} - {product.name} (reservation)",
                ...
                'location_id': location_src.id,
                'location_dest_id': location_dest.id,
                'material_order_id': self.id,
                'property_id': self.property_id.id if self.property_id else False,
            })
            reserved_lines.append(f"{product.name} x {qty}")
        
        if picking.move_ids_without_package:
            picking.action_confirm()
            picking.action_assign()
            self.stock_reserved = True
            self.message_post(body=f"<b>Stock reserved:</b> {', '.join(reserved_lines)}")
        else:
            picking.sudo().unlink()
            self.message_post(body="<b>Stock reservation failed:</b> No storable products found.")
    ...
    def _compute_escrow_account(self):
        for record in self:
            ...
            if record.escrow_company:
                escrow_account = self.env['account.account'].with_company(record.escrow_company).search([
    ...
    def notify_when_special_order_approved(self):
        ...
        if self.special_order_approved:  # Check if true
            ...
            if 'sms.silvamedia' not in self.env:
                return
    ...
    def create_bill(self): # NEEDS TESTING
        ...
        if self.has_bill == True:
            ...
            if self.property_id.on_hold == True and self.order_status == 'not_ordered':
                _logger.warning(f"Property {self.property_id.name} is on hold but proceeding with bill creation for material order {self.name}")
                self.message_post(body="<b>Warning:</b> This property is on hold. Bill created despite on-hold status.")
            company = self.env['res.company'].search([('partner_id', '=', self.property_id.partner_id.id)], limit=1).id
            repair_account = self.env['account.account'].search([('code', '=', REPAIR_ACCOUNT_NUMBER)], limit=1).id
            ...
            if not self.payment_terms or not self.payment_terms.line_ids:
                ...
                due_date = fields.Date.today() + timedelta(days=self.payment_terms.line_ids[0].nb_days)
            if not company and self.paid_with_other_company == False:
                ...
                escrow_account = self.env['account.account'].search([('code', '=', ESCROW_ACCOUNT_NUMBER)], limit=1).id
    ...
    def create_records_paid_with_other_company(self):
        ...
        main_journal_entry_lines = [
            (0, 0, {
                ...
                'account_id': self.env['account.account'].search([('code', '=', ACCOUNT_PAYABLE_NUMBER)], limit=1).id,
            ...
            (0, 0, {
                ...
                'account_id': self.env['account.account'].search([('code', '=', LOAN_BETWEEN_RELATED_COMPANIES_NUMBER)], limit=1).id,
...
#############################################################################################################################################################################################################
    ...
    def ordered(self): # NEEDS TESTING
        ...
        # On-hold warning (no longer blocks the operation)
        if self.property_id.on_hold == True and self.order_status == 'not_ordered':
            _logger.warning(f"Property {self.property_id.name} is on hold but proceeding with material order {self.name}")
            self.message_post(body="<b>Warning:</b> This property is on hold. Material order processed despite on-hold status.")
...
###########################################################################################################################################################################################################
 ...
 # Nueva funcion para manejar solo emails
    ...
    def _send_sms_messages(self, message): # NEEDS TESTING
        self.ensure_one()
        if 'sms.silvamedia' not in self.env:
            _logger.warning('sms.silvamedia module not installed, skipping SMS')
            return
        access_token = self.env['sms.silvamedia'].search([], limit=1).access_token
        location_id = self.env['sms.silvamedia'].search([], limit=1).location_id
...
##################################################################################################################################################################################################################################################################################################################
    ...
    def _get_storable_product(self, name, uom_id=None):
        """Find an existing storable product by name, or create one for inventory tracking."""
        Product = self.env['product.product']
        product = Product.search([('name', '=', name), ('type', '=', 'consu'), ('is_storable', '=', True)], limit=1)
        if product:
            return product

        uom = uom_id or self.env.ref('uom.product_uom_unit', raise_if_not_found=False)
        ...
        _logger.info(f"Auto-created storable product '{name}' (id={product.id}) for inventory tracking")
        return product

    def _resolve_product_for_stock(self, line):
        """Resolve a material line to a storable product.product for stock moves."""
        product = line.product
        uom_ref = None

        if line.unit_measure:
            uom_ref = self.env['uom.uom'].search([('name', '=ilike', line.unit_measure)], limit=1)

        if product:
            if product.type == 'product':
                return product
            if product.type in ('consu', 'service'):
                storable = self._get_storable_product(product.name, uom_ref or product.uom_id)
                return storable

        if line.subproduct:
            return self._get_storable_product(line.subproduct.name, uom_ref)

        return False

    def action_retry_inventory_deduction(self):
        """Manual retry button for orders already delivered but not deducted."""
        self.ensure_one()
        if self.inventory_deducted:
            raise UserError(_("Inventory has already been deducted for this order."))
        if self.order_status != 'delivered':
            raise UserError(_("Only delivered orders can have inventory deducted."))
        self._create_stock_picking()
        if not self.inventory_deducted:
            raise UserError(_(
                "Inventory deduction failed. Possible reasons:\n"
                "- No material lines on this order\n"
                "- Material lines have no product or subproduct set\n"
                "- All quantities are zero\n"
                "Check the chatter/log for details."
            ))

    def _create_stock_picking(self):
        if self.inventory_deducted:
            _logger.info(f"Inventory already deducted for material order {self.name}")
            return
        
        if not self.material_lines:
            _logger.warning(f"No material lines found for material order {self.name}")
            self.message_post(body="Inventory deduction skipped: no material lines found on this order.")
            return
        ...
        warehouse = self.env['stock.warehouse'].search([('company_id', '=', self.env.company.id)], limit=1)
        if not warehouse:
            raise UserError(_("No warehouse found for the current company. Please configure a warehouse first."))
        ...
        if not location_dest:
            ...
            if not location_dest:
                raise UserError(_("Customer location not found. Please configure stock locations."))
        ...
        picking_vals = {
            ...
            'location_id': location_src.id,
            'location_dest_id': location_dest.id,
            'origin': self.name or 'Material Order',
            'partner_id': self.property_owner.id if self.property_owner else False,
            'scheduled_date': fields.Datetime.now(),
        }
        ...
        for line in self.material_lines:
            ...
            if not product:
                line_name = line.subproduct.name if line.subproduct else (line.product.name if line.product else 'Unknown')
                skipped_lines.append(f"'{line_name}': no product or subproduct set")
                continue
            
            move_vals = {
                'name': f"{self.name or 'MO'} - {product.name}",
                'product_id': product.id,
                'product_uom_qty': qty,
                'product_uom': product.uom_id.id,
                'picking_id': picking.id,
            ...
            StockMove.sudo().create(move_vals)
            processed_lines.append(f"'{product.name}' x {qty}")
        ...
        if picking.move_ids_without_package:
            try:
                ...
                result = picking.sudo().button_validate()
                if isinstance(result, dict) and result.get('res_model'):
                    wizard_model = result['res_model']
                    wizard_id = result.get('res_id')
                    if wizard_id:
                        wizard = self.env[wizard_model].sudo().browse(wizard_id)
                    else:
                        ctx = result.get('context', {})
                        wizard = self.env[wizard_model].sudo().with_context(**ctx).create({})
                ...
                message = f"<b>Inventory deducted successfully.</b> Picking: {picking.name}<br/>"
                message += f"<b>Processed:</b> {', '.join(processed_lines)}"
                if skipped_lines:
                    message += f"<br/><b>Skipped:</b> {', '.join(skipped_lines)}"
                self.message_post(body=message)
                _logger.info(f"Stock picking {picking.name} validated for material order {self.name}")
            except Exception as e:
                _logger.error(f"Error validating stock picking for {self.name}: {str(e)}", exc_info=True)
                self.message_post(body=f"<b>Inventory deduction error:</b> {str(e)}")
                if picking.state == 'draft':
                    picking.sudo().unlink()
                raise UserError(_(f"Error during inventory deduction: {str(e)}"))
        else:
            picking.sudo().unlink()
            skip_detail = ", ".join(skipped_lines) if skipped_lines else "No products/subproducts on any line"
            msg = f"<b>Inventory deduction failed:</b> No stock moves could be created. {skip_detail}"
            self.message_post(body=msg)
            _logger.warning(f"No valid stock moves for material order {self.name}: {skip_detail}")
    ...
    def open_action(self):
        action_view = self.env.ref('pms.pms_materials_view_form')
        return {
    ...
    def action_view_stock_moves(self):
        ...
        action['domain'] = [('material_order_id', '=', self.id)]
        action['context'] = {
            'search_default_material_order_id': self.id,
            'default_material_order_id': self.id,
