...
class AccountMove(models.Model):
    ...
    related_invoice_ids = fields.Many2many(
        ...
        relation="account_move_invoice_bill_rel",
        column1="bill_id",
        column2="invoice_id",
        string="Related Invoices",
        domain="[('move_type', '=', 'out_invoice')]",
        help="Invoices related to this bill"
    )
    related_bill_ids = fields.Many2many(
        comodel_name="account.move",
        relation="account_move_invoice_bill_rel",
        column1="invoice_id",
        column2="bill_id",
        ...
        help="Bills related to this invoice"
    )
    
    total_related_sales = fields.Float(
        string="Total Related Sales",
        compute="_compute_related_totals",
        store=True,
        help="Sum of all related invoice amounts"
    )
    total_related_purchases = fields.Float(
        string="Total Related Purchases",
        compute="_compute_related_totals",
        store=True,
        help="Sum of all related bill amounts"
    )
    profit_amount = fields.Float(
        string="Profit",
        compute="_compute_related_totals",
        store=True,
        help="Total Sales - Total Purchases"
    )
    ...
    invoice_payment_classification = fields.Selection(
        selection=[
            ('completed_work', 'On Hold (Completed Work)'),
        ...
        string="Payment Classification",
        compute="_compute_payment_classification",
        inverse="_inverse_payment_classification",
        store=True,
        readonly=False,
        help="Auto-calculated from payment terms, but can be manually overridden. "
             "Only 'Completed Work' invoices trigger on-hold alerts."
    )
    triggers_on_hold = fields.Boolean(
        string="Triggers On-Hold Alerts",
        compute="_compute_triggers_on_hold",
        store=True,
        help="True if this invoice will trigger on-hold alerts when overdue"
    )
    is_advance_payment = fields.Boolean(
        string="Is Advance/Prepaid",
        compute="_compute_triggers_on_hold",
        store=True,
        help="True for advance payments — work not yet performed. These never trigger on-hold."
    )
    
    def _inverse_payment_classification(self):
        pass

    @api.depends('invoice_payment_classification')
    def _compute_triggers_on_hold(self):
        for record in self:
            record.triggers_on_hold = record.invoice_payment_classification == 'completed_work'
            record.is_advance_payment = record.invoice_payment_classification == 'advance_payment'

    @api.depends('invoice_payment_term_id', 'invoice_payment_term_id.anticipated_payment', 
                 'invoice_payment_term_id.utility_payment', 'invoice_payment_term_id.material_payment',
                 'linked_material_order', 'move_type')
    def _compute_payment_classification(self):
        for record in self:
            if record.move_type not in ('out_invoice', 'in_invoice'):
                record.invoice_payment_classification = 'na'
                continue
            
            if record.linked_material_order:
                record.invoice_payment_classification = 'material_order'
            elif record.invoice_payment_term_id:
                term = record.invoice_payment_term_id
                if term.anticipated_payment:
                    record.invoice_payment_classification = 'advance_payment'
                elif term.utility_payment:
                    record.invoice_payment_classification = 'utility'
                elif term.material_payment:
                    record.invoice_payment_classification = 'material_payment'
                else:
                    record.invoice_payment_classification = 'completed_work'
            else:
                record.invoice_payment_classification = 'completed_work'
    utility_pin = fields.Char(string="Utility Pin")
    invoice_link_docs = fields.Char(string="Invoice Link", store=True)
    payment_type_bills = fields.Selection([("check", "Check/ACH"), ("online", "Online / CC"), ("material", "Material")], string="Payment Type (Bills)", store=True, default=False)
    linked_activities = fields.Many2one("pms.projects.routes", string="Linked Activity")
    linked_activities_end_date = fields.Datetime(related="linked_activities.end_date", string="Activity End Date", store=True, readonly=True)
    ...
    def _compute_cash_match_date(self):
        ...
        for line in lines_to_process:
            ...
            if line.matching_number != False:
                ...
                final_dates.pop(line.move_id.id, None)
    ...
    @api.depends('related_invoice_ids', 'related_invoice_ids.amount_total_signed', 'related_bill_ids', 'related_bill_ids.amount_total_signed')
    def _compute_related_totals(self):
        for record in self:
            if record.move_type == 'in_invoice':
                record.total_related_sales = sum(record.related_invoice_ids.mapped('amount_total_signed'))
                record.total_related_purchases = abs(record.amount_total_signed)
                record.profit_amount = record.total_related_sales - record.total_related_purchases
                record.profit_margin = (record.profit_amount / record.total_related_sales * 100) if record.total_related_sales != 0 else 0.0
            elif record.move_type == 'out_invoice':
                record.total_related_sales = abs(record.amount_total_signed)
                record.total_related_purchases = sum(abs(bill.amount_total_signed) for bill in record.related_bill_ids)
                record.profit_amount = record.total_related_sales - record.total_related_purchases
                record.profit_margin = (record.profit_amount / record.total_related_sales * 100) if record.total_related_sales != 0 else 0.0
            else:
                record.total_related_sales = 0.0
                record.total_related_purchases = 0.0
...
# 
    ...
    @api.depends('property_id.status_property', 'property_id.on_hold')
    def _compute_lock_on_coc(self):
        for record in self:
            if record.property_id and record.property_id.status_property == 'coc' and not record.property_id.on_hold:
    ...
    def _compute_property_id(self):
        ...
        for record in self:
            if record.analytic_accounts:
                all_names.update(split_properties(record.analytic_accounts))

        prop_by_name = {}
        if all_names:
            props = self.env['pms.property'].search([('analytical_account.name', 'in', list(all_names))])
            for prop in props:
                if prop.analytical_account and prop.analytical_account.name:
                    prop_by_name[prop.analytical_account.name] = prop

        for record in self:
            record.property_id = False
            if record.analytic_accounts:
                for property_name in split_properties(record.analytic_accounts):
                    matched = prop_by_name.get(property_name)
                    if matched:
                        record.property_id = matched
                        break
            ...
            for line in record.line_ids:
                if line.analytic_distribution:
                    for key in line.analytic_distribution.keys():
                        try:
                            all_analytic_ids.add(int(key))
                        except (ValueError, TypeError):
                            continue

        account_names = {}
        if all_analytic_ids:
            accounts = self.env["account.analytic.account"].browse(list(all_analytic_ids)).exists()
            account_names = {acc.id: acc.name for acc in accounts if acc.name}
                ...
                if line.analytic_distribution:
                    for key in line.analytic_distribution.keys():
                        try:
                            ...
                            continue
                        name = account_names.get(key_int)
                        if name:
                            address_final.add(name)

            record.analytic_accounts = ",".join(sorted(address_final)) if address_final else False
    ...
    def _get_last_customer_message(self):
        ...
        msg_by_id = {}
        for msg in messages:
            if msg.res_id not in msg_by_id:
                msg_by_id[msg.res_id] = msg.body

        for record in self:
            record.last_customer_message = msg_by_id.get(record.id, "")
    ...
    def check_off_hold(self, property_obj):
        """
        Check if a property that is currently on hold can be taken off hold.
        
        A property can be taken off hold when:
        - There are NO overdue invoices for completed work (3+ days past due)
        
        Invoices that do NOT prevent off-hold:
        - Prepaid/Advance Payment invoices (anticipated_payment = True)
        - Utility Payment invoices (utility_payment = True)  
        - Material Payment invoices (material_payment = True)
        - Invoices in 'in_payment' status (payment is being processed)
        
        Only invoices for COMPLETED WORK that are UNPAID and OVERDUE will keep property on hold.
        """
    ...
    def check_on_hold(self):
        """
        Automated daily check to put properties on hold when invoices for completed work are overdue.
        
        ON HOLD LOGIC:
        - Triggers when invoices for COMPLETED WORK are 4+ days overdue
        - Only applies to 3rd party properties (own_third = 'third')
        - Only for non-custodial projects
        
        EXCLUDED from triggering on-hold (Prepaid/Special payments):
        - Prepaid/Advance Payment invoices (anticipated_payment = True)
        - Utility Payment invoices (utility_payment = True)
        - Material Payment invoices (material_payment = True)
        - Material order linked invoices
        - Credit notes/refunds
        
        This ensures that only invoices for work already completed will trigger on-hold alerts,
        while advance payments and special payment types are tracked separately without alerts.
        """
        ...
        query = """
            ...
            AND account_move.company_id IN (42, 46, 47, 48, 49)
    ...
    def open_intercompany_payment(self):
        return {
            ...
            'context': {'default_company_id': self.company_id.id, 
                        'default_amount_total': abs(self.amount_total_signed),
                        'default_amount_due': abs(self.amount_residual),
...
###################################################################################################################################################################################################################################################################################
    ...
    def action_post(self):
        # moves_with_payments = self.filtered('payment_id') # FIXME: Odoo 18 compatibility
        other_moves = self # FIXME: Odoo 18 compatibility - was: self - moves_with_payments

        out_invoices = self.filtered(lambda m: m.move_type == "out_invoice")
        if out_invoices:
            analytic_key_ids = set()
            for move in out_invoices:
                for line in move.line_ids:
                    if line.analytic_distribution:
                        for key in line.analytic_distribution.keys():
                            try:
                                analytic_key_ids.add(int(key))
                            except (ValueError, TypeError):
                                continue

            coc_warning_needed = False
            if analytic_key_ids:
                properties = self.env["pms.property"].search([("analytical_account", "in", list(analytic_key_ids))])
                if properties:
                    projects = self.env["pms.projects"].search([
                        ("address", "in", properties.ids),
                        ("status_construction", "in", ["coc", "completed"]),
                    ])
                    if projects:
                        coc_warning_needed = True

            if coc_warning_needed:
                self.env['bus.bus']._sendone(self.env.user.partner_id, 'simple_notification', {
                    'type': 'danger',
                    'title': _("Warning"),
        ...
        if False: # FIXME: Odoo 18 compatibility - was: if moves_with_payments:
            pass
            # moves_with_payments.payment_id.action_post() # FIXME: Odoo 18 compatibility
        if other_moves:
...
# Checking property and product in the invoice lines
    def closing_check(self):
        for move in self:
            ...
            if all_lines:
                address_final = []
                filtered_lines = all_lines.filtered_domain(["&", ("analytic_distribution", "!=", False), ("account_id.code", "=", "1000201")])
                for analytic_id in filtered_lines:
                    ...
                    if analytic_i:
                        for key in analytic_i.keys():
                            try:
                                ...
                                continue
                            account = move.env["account.analytic.account"].sudo().browse(key_int)
                            if account.exists():
                                address_final.append(account.id)
                
                # Warn about closed properties but do not block the operation
                if address_final:
                    ...
                    if pms_address:
                        ...
                        if True in closing_check:
                            closed_names = pms_address.filtered(lambda p: p.residential_unit_closure).mapped("name")
                            _logger.warning(f"Invoice {move.name}: properties already closed: {', '.join(closed_names)}")
                            move.message_post(body=f"<b>Warning:</b> The following properties are closed/residential units: {', '.join(closed_names)}. Invoice posted despite closure status.")
                return True
    ...
    def action_auto_link_bills_invoices(self):
        linked_count = 0
        total_links = 0
        
        for move in self:
            if move.move_type == 'in_invoice':
                invoices_to_link = self._find_matching_invoices(move)
                if invoices_to_link:
                    move.related_invoice_ids = [(4, inv.id) for inv in invoices_to_link]
                    linked_count += 1
                    total_links += len(invoices_to_link)
            elif move.move_type == 'out_invoice':
                bills_to_link = self._find_matching_bills(move)
                if bills_to_link:
                    move.related_bill_ids = [(4, bill.id) for bill in bills_to_link]
                    linked_count += 1
                    total_links += len(bills_to_link)
        
        if total_links > 0:
            message = f'Successfully linked {linked_count} document(s) with {total_links} related document(s).'
            msg_type = 'success'
        else:
            message = 'No matching documents found. Criteria: same products, same analytic accounts, or within 90 days with matching partner.'
            msg_type = 'warning'
    ...
    def _find_matching_invoices(self, bill):
        domain = [
            ('move_type', '=', 'out_invoice'),
            ('state', '=', 'posted'),
            ('id', 'not in', bill.related_invoice_ids.ids),
        ]
        
        bill_products = bill.invoice_line_ids.filtered(lambda l: l.product_id).mapped('product_id')
        bill_analytics = self._get_analytic_accounts(bill)
        
        if not bill_analytics:
            return self.env['account.move']
        
        invoice_candidates = self.env['account.move'].search(domain)
        matching_invoices = self.env['account.move']
        
        for invoice in invoice_candidates:
            invoice_products = invoice.invoice_line_ids.filtered(lambda l: l.product_id).mapped('product_id')
            invoice_analytics = self._get_analytic_accounts(invoice)
            
            has_common_analytics = bool(invoice_analytics and (bill_analytics & invoice_analytics))
            if not has_common_analytics:
                continue
            
            has_common_products = bool(bill_products & invoice_products)
            
            date_diff = abs((invoice.invoice_date - bill.invoice_date).days) if invoice.invoice_date and bill.invoice_date else 999
            within_close_date_range = date_diff <= 30
            
            amount_diff = abs(abs(bill.amount_total) - abs(invoice.amount_total))
            amount_close_match = amount_diff <= max(1.0, abs(bill.amount_total) * 0.05)
            
            if amount_close_match:
                matching_invoices |= invoice
            elif has_common_products and within_close_date_range:
                matching_invoices |= invoice
        
        return matching_invoices
    
    def _find_matching_bills(self, invoice):
        domain = [
            ('move_type', '=', 'in_invoice'),
            ('state', '=', 'posted'),
            ('id', 'not in', invoice.related_bill_ids.ids),
        ]
        
        invoice_products = invoice.invoice_line_ids.filtered(lambda l: l.product_id).mapped('product_id')
        invoice_analytics = self._get_analytic_accounts(invoice)
        
        if not invoice_analytics:
            return self.env['account.move']
        
        bill_candidates = self.env['account.move'].search(domain)
        matching_bills = self.env['account.move']
        
        for bill in bill_candidates:
            bill_products = bill.invoice_line_ids.filtered(lambda l: l.product_id).mapped('product_id')
            bill_analytics = self._get_analytic_accounts(bill)
            
            has_common_analytics = bool(bill_analytics and (invoice_analytics & bill_analytics))
            if not has_common_analytics:
                continue
            
            has_common_products = bool(invoice_products & bill_products)
            
            date_diff = abs((invoice.invoice_date - bill.invoice_date).days) if invoice.invoice_date and bill.invoice_date else 999
            within_close_date_range = date_diff <= 30
            
            amount_diff = abs(abs(invoice.amount_total) - abs(bill.amount_total))
            amount_close_match = amount_diff <= max(1.0, abs(invoice.amount_total) * 0.05)
    ...
    def _get_analytic_accounts(self, move):
        analytic_accounts = set()
        for line in move.invoice_line_ids:
            if line.analytic_distribution:
                for acc_id in line.analytic_distribution.keys():
                    analytic_accounts.add(int(acc_id))
...
class AccountMoveSendInherit(models.AbstractModel):
    ...
    def _get_invoice_extra_attachments(self, move):
        ...
        extra_attachments = self.env['ir.attachment'].search([
            ('res_model', '=', 'account.move'),
            ('res_id', '=', move.id),
            ('id', '!=', move.invoice_pdf_report_id.id if move.invoice_pdf_report_id else 0),
        ])
        attachments |= extra_attachments

        if hasattr(move, 'linked_contractor_job') and move.linked_contractor_job:
            job_attachments = self.env['ir.attachment'].search([
                ('res_model', '=', 'pms.contractor.job'),
                ('res_id', '=', move.linked_contractor_job.id),
            ])
