...
class pms_loans(models.Model):
    ...
    interest = fields.Float(string="Interest Rate", digits=(12, 6))
    ...
    total_drawed_amount = fields.Monetary(string="Drawed Amount", currency_field='company_currency_id', compute="_calculate_total_drawed", store=False)
    ...
    def _calculate_total_drawed(self):
        for record in self:
            drawed_amount = record.env["pms.draws"].sudo().search([("loan_id", "=", record.id)])
            record.total_drawed_amount = sum(drawed_amount.sudo().mapped("draw_amount"))
    ...
    def _calculate_balance(self):
        for record in self:
            ...
            record.available_balance = record.loan_amount - drawed_amount
