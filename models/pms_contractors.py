...
class pms_contractor_job(models.Model):
    ...
    legacy_job_creation_id = fields.Many2one('pms.job.creation', string='Migrated From (Job Creation)', readonly=True,
        help="If set, this contractor job was migrated from the legacy Job Creation module.")

    name = fields.Char(string='Job Name', required=True)
    description = fields.Char(string='Job Description')
    deadline = fields.Date(string='Deadline')
    maintenance_order = fields.Boolean(string='No Invoice Needed')
    ...
    def create(self, vals_list):
        ...
        for record in records:
            if not record.linked_project_activity and record.property_id and record.contractor_id:
                record._auto_link_project_activity()
        return records
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            if 'contractor_id' in vals and record.linked_project_activity:
                if record.contractor_id and record.linked_project_activity.vendor != record.contractor_id:
                    record.linked_project_activity.vendor = record.contractor_id
                    _logger.info(f"Synced vendor {record.contractor_id.name} to activity {record.linked_project_activity.id}")
            if 'linked_project_activity' not in vals and not record.linked_project_activity:
                if record.property_id and record.contractor_id:
                    record._auto_link_project_activity()
        return res

    def _auto_link_project_activity(self):
        self.ensure_one()
        project = self.env['pms.projects'].search([('address', '=', self.property_id.id)], limit=1)
        if not project:
            return
        domain = [
            ('project_property', '=', project.id),
            ('completed', '=', False),
            ('contractor_job_ids', '=', False),
        ]
        if self.contractor_id:
            activity = self.env['pms.projects.routes'].search(
                domain + [('vendor', '=', self.contractor_id.id)], limit=1
            )
            if activity:
                self.linked_project_activity = activity.id
                _logger.info(f"Auto-linked contractor job {self.id} to activity {activity.id} (vendor match)")
                return
        activity = self.env['pms.projects.routes'].search(
            domain + [('vendor', '=', False)], limit=1, order='sequence asc'
        )
        if activity:
            self.linked_project_activity = activity.id
            if self.contractor_id:
                activity.vendor = self.contractor_id
            _logger.info(f"Auto-linked contractor job {self.id} to activity {activity.id} (unassigned)")
    ...
    def complete_job(self):
        for record in self:
            ...
            # Sync completion to linked project activity
            if record.linked_project_activity and not record.linked_project_activity.completed:
                record.linked_project_activity._complete_jobs()
                _logger.info(f"Auto-completed project activity {record.linked_project_activity.id} from contractor job {record.id}")
        return
