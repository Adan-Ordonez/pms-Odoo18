...
class pms_projects_routes(models.Model):
    ...
    contractor_job_ids = fields.One2many(
        ...
        string='Contractor Jobs',
        help='Contractor jobs linked to this project activity.'
    )
    contractor_job_count = fields.Integer(
        string='Contractor Jobs',
        compute='_compute_contractor_job_count',
    )
    contractor_job_state = fields.Selection(
        [('created', 'Created'), ('pending', 'Pending'), ('ordered', 'Ordered'),
         ('in_progress', 'In Progress'), ('completed', 'Completed'), ('cancelled', 'Cancelled')],
        string='Contractor Job Status',
        compute='_compute_contractor_job_state',
    )

    @api.depends('contractor_job_ids', 'contractor_job_ids.state')
    def _compute_contractor_job_count(self):
        for record in self:
            record.contractor_job_count = len(record.contractor_job_ids)

    @api.depends('contractor_job_ids', 'contractor_job_ids.state')
    def _compute_contractor_job_state(self):
        for record in self:
            active_jobs = record.contractor_job_ids.filtered(lambda j: j.state != 'cancelled')
            if active_jobs:
                record.contractor_job_state = active_jobs[0].state
            else:
                record.contractor_job_state = False

    def action_view_contractor_jobs(self):
        """Open the linked contractor jobs."""
        self.ensure_one()
        if self.contractor_job_count == 1:
            return {
                'type': 'ir.actions.act_window',
                'name': 'Contractor Job',
                'res_model': 'pms.contractor.job',
                'view_mode': 'form',
                'res_id': self.contractor_job_ids[0].id,
                'target': 'current',
            }
        return {
            'type': 'ir.actions.act_window',
            ...
            'res_model': 'pms.contractor.job',
            'view_mode': 'list,form',
            'domain': [('id', 'in', self.contractor_job_ids.ids)],
            'target': 'current',
        }

    def action_create_contractor_job(self):
        """Create a contractor job from this project activity."""
        self.ensure_one()
        property_id = self.address.id if self.address else False
        vals = {
            'property_id': property_id,
            'name': self.product.name if self.product else (self.name.name if self.name else 'Job'),
            'contractor_id': self.vendor.id if self.vendor else False,
            'linked_project_activity': self.id,
            'linked_activity': self.name.activity.id if self.name and self.name.activity else False,
            'linked_product': self.product.id if self.product else False,
            'deadline': self.expected_end_date.date() if self.expected_end_date else False,
        }
        job = self.env['pms.contractor.job'].create(vals)
        _logger.info(f"Created contractor job {job.id} from project activity {self.id}")
        return {
            'type': 'ir.actions.act_window',
            'name': 'Contractor Job',
            'res_model': 'pms.contractor.job',
    ...
    invoice_ids = fields.One2many(
        "account.move", 
        "linked_activities", 
        string="Linked Activity Invoices",
        help="Invoices linked to this activity."
    )
    
    bills = fields.Many2many(
        "account.move",
        string="Bills",
        compute="_compute_bills",
        store=False,
        help="All bills related to this activity (linked activities + contractor bills)."
    )
    
    @api.depends('invoice_ids', 'vendor', 'address', 'contractor_job_ids', 'contractor_job_ids.linked_bill', 'contractor_job_ids.linked_invoice')
    def _compute_bills(self):
        for record in self:
            # Get bills from linked_activities
            linked_bills = record.invoice_ids
            
            # Get contractor bills for this property and vendor
            contractor_bills = self.env['account.move'].search([
                ('contractor', '=', record.vendor.id),
                ('property_id', '=', record.address.id),
                ('move_type', '=', 'in_invoice'),
                ('linked_activities', '=', False),
            ]) if record.vendor and record.address else self.env['account.move']
            
            # Get bills/invoices from linked contractor jobs
            job_moves = self.env['account.move']
            for job in record.contractor_job_ids:
                if job.linked_bill:
                    job_moves |= job.linked_bill
                if job.linked_invoice:
                    job_moves |= job.linked_invoice
            
            # Combine all, remove duplicates
            record.bills = linked_bills | contractor_bills | job_moves    
    ...
    def _check_project_on_hold(self):
        ...
        for record in self:
            ...
            if not is_name_excluded and record.project_property and record.project_property.on_off_hold:
                _logger.warning(f"Project {record.project_property.name} is on hold but proceeding with activity creation: {record.name}")
    ...
    def _complete_jobs(self): # optimized for one single write
        for record in self:
            ...
            # Sync completion to linked contractor jobs
            for job in record.contractor_job_ids:
                if job.state not in ('completed', 'cancelled'):
                    ...
                    if job.bill_created and job.linked_bill:
                        job.linked_bill.activity_completed = True
                    _logger.info(f"Auto-completed contractor job {job.id} from activity {record.id}")
    ...
    def create(self, vals_list):
        _logger.info("Activity create method called with vals: %s", vals_list)
        records = super(pms_projects_routes, self).create(vals_list)
        _logger.info("New activity records created with IDs: %s", records.ids)
        records.activity_check()
        return records
    ...
    def write(self, vals):
        ...
        self.activity_check()
        # Sync vendor changes to linked contractor jobs
        if 'vendor' in vals:
            for record in self:
                for job in record.contractor_job_ids:
                    if job.state not in ('completed', 'cancelled') and job.contractor_id.id != record.vendor.id:
                        job.contractor_id = record.vendor.id
                        _logger.info(f"Synced vendor {record.vendor.name} to contractor job {job.id}")
        return res
