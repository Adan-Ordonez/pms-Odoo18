...
class pms_property(models.Model):
    ...
    # === Budget Id === #
    #budget_ids = fields.Many2one("budget.budget")
    ...
    def put_off_hold(self, auto=False):
        ...
        for record in self:
            _logger.info("put_off_hold: processing property %s (id=%s)", record.name, record.id)
            ...
            try:
                _logger.info("put_off_hold: setting on_hold=False")
                record.on_hold = False
                _logger.info("put_off_hold: on_hold set OK")
            except Exception as e:
                _logger.error("put_off_hold: FAILED on on_hold=False: %s", e, exc_info=True)
                raise
            try:
                _logger.info("put_off_hold: setting date_last_set_on_hold=False")
                record.date_last_set_on_hold = False
                _logger.info("put_off_hold: date_last_set_on_hold set OK")
            except Exception as e:
                _logger.error("put_off_hold: FAILED on date_last_set_on_hold=False: %s", e, exc_info=True)
                raise
    ...
    def _get_document_folder(self):
        try:
            return self.env["documents.document"].search([("name", "=", "Properties"), ("type", "=", "folder")], limit=1)
        except Exception:
            return False
    ...
    #             property_budget = records.env["budget.budget"].sudo().browse(model_budget_id).sudo().copy(
    #             {"name": records.name,
